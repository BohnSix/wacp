...
from opencood.models.fuse_modules.fusion_in_one import MaxFusion, AttFusion, DiscoFusion, V2VNetFusion, V2XViTFusion, When2commFusion, MKDCooperFusion
from opencood.utils.transformation_utils import normalize_pairwise_tfm

class PointPillarBaselineCommunication(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(PointPillarBaselineCommunication, self).__init__()
        ...
        if communication_mode == 'ermvp':
            ...
            self.communication_network = ERMVPCommunication(channels=64, topk_ratio=0.1, cluster_sample_ratio=0.2, out_channels=64)
            ...
            # self.communication_network = W2CCommunication(topk_ratio=0.1, use_smooth=True, use_ste=False, out_channels=64)
            self.communication_network = W2CCommunication(topk_ratio=0.1, use_smooth=True, use_ste=False, out_channels=64)
        ...
        if args['fusion_method'] == 'mkdcooper':
            self.fusion_net = MKDCooperFusion(args['mkdcooper'])
        ic('Using fusion method: {}'.format(args['fusion_method']))
    ...
    def forward(self, data_dict):
        ...
        if self.communication_mode == 'ermvp':
            ...
            spatial_features_2d = self.communication_network.communication(x=spatial_features_2d, record_len=record_len, cls_head=self.cls_head)
        elif self.communication_mode == 'where2comm':
            # Use the Where2Comm communication network
            spatial_features_2d = self.communication_network(spatial_features_2d, record_len, self.cls_head)
        elif self.communication_mode == 'naivecomm':
            # Use the Naive Communication Network
            spatial_features_2d = self.communication_network(spatial_features_2d, record_len)
        ### end
