...
class PointPillarBaseline(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.shrink_flag:
            spatial_features_2d = self.shrink_conv(spatial_features_2d) # torch.Size([2, 256, 100, 352])

        if self.compression:
            spatial_features_2d = self.naive_compressor(spatial_features_2d)

        ## For debugging of single perception, you can uncomment the following lines to test with a single sample#
        # spatial_features_2d = spatial_features_2d[0:1]
        # record_len = torch.tensor([1], device='cuda:0')
