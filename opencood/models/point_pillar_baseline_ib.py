...
from opencood.extensions.coib_communication_baseline import IBCommunication 


class PointPillarBaselineIB(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(PointPillarBaselineIB, self).__init__()

        feature_map_size = args['point_pillar_scatter']['grid_size'].tolist()[:2][::-1]

        ### coib communication
        self.ib_communication = IBCommunication(channels=64, encoding_dim=256, decoding_dim=256, output_size=feature_map_size, use_ego_feature=True, out_channels=64)
        print(self.ib_communication)
        ### end
    ...
    def forward(self, data_dict):
        ...
        ### coib communication
        spatial_features_2d, KL_value, RL_value = self.ib_communication(x=spatial_features_2d, record_len=record_len, cls_head=self.cls_head, data_dict=data_dict, required_KL=True)
        ### end
