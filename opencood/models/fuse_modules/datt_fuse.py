...
class MultiDilatelocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x_clone = x.clone()
        for i in range(self.num_dilation):
            x_clone[i] = self.dilate_attention[i](qkv[i][0], qkv[i][1], qkv[i][2])  # B, H, W,C//num_dilation
        x_clone = x_clone.permute(1, 2, 3, 0, 4).reshape(B, H, W, C)
        x_clone = self.proj(x_clone)
        x_clone = self.proj_drop(x_clone)
        return x_clone.permute(0, 3, 1, 2)

if __name__ == "__main__":
    input = torch.rand([3, 64, 200, 704])
    model = MultiDilatelocalAttention(64)
    output = model(input)
    print(output.shape)