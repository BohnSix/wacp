...
from opencood.models.fuse_modules.datt_fuse import MultiDilatelocalAttention
from opencood.models.fuse_modules.focal_fuse import FocalVehicleModulation
...
class MKDCooperFusion(nn.Module):
    def __init__(self, args):
        super(MKDCooperFusion, self).__init__()
...
class DattFusion(nn.Module):
    def __init__(self, feature_dims, fuse_mode='mean'):
        super(DattFusion, self).__init__()
        self.att = MultiDilatelocalAttention(feature_dims)
    ...
    def forward(self, xx, record_len, normalized_affine_matrix):
        ...
        for b in range(B):
            ...
            if self.fuse_mode == 'mean':
                out.append(h.mean(dim=0, keepdim=False))
            elif self.fuse_mode == 'max':
                out.append(h.max(dim=0, keepdim=False)[0])
            else:
                pass
...
class FocalFusion(nn.Module):
    def __init__(self, feature_dims, fuse_mode='gate'):
        super(FocalFusion, self).__init__()
        self.att = FocalVehicleModulation(dim=feature_dims, focal_window=3, focal_level=2)
        self.fuse_mode = fuse_mode
        print("FocalFusion")
