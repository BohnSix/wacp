...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        self.load_lidar_file = (
            True if "lidar" in params["input_source"] or self.visualize else False
        )
        ...
        for i, scenario_folder in enumerate(self.scenario_folders):
            ...
            for j, cav_id in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    depth_files = self.find_camera_files(
            ...
            if os.path.exists(hdf5_file):
                ...
                if self.load_camera_file:
                    data[cav_id]["camera_data"] = load_camera_data(
                        cav_content[timestamp_key]["cameras"]
                    )
                if self.load_depth_file:
                    data[cav_id]["depth_data"] = load_camera_data(
                        cav_content[timestamp_key]["depths"]
                    )
    ...
    def get_ext_int(self, params, camera_id):
        ...
        camera_to_lidar = x1_to_x2(camera_coords, params["lidar_pose_clean"]).astype(
            np.float32
        )  # T_LiDAR_camera
