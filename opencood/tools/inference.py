...
from opencood.visualization import simple_vis_new as simple_vis
...
def test_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="opencood/logs/all_ablation_study/test_where2comm/opv2v_coalign_comm_002",
    ...
    parser.add_argument(
        "--no_score", action="store_true", help="whether print the score of prediction"
    )
...
def main():
    ...
    assert opt.fusion_method in ["late", "early", "intermediate", "no", "single"]

    hypes = yaml_utils.load_yaml(None, opt)

    hypes["validate_dir"] = hypes["test_dir"]
    if "OPV2V" in hypes["test_dir"].upper() or "v2xsim" in hypes["test_dir"].upper():
    ...
    left_hand = (
        True
        if (
            "OPV2V" in hypes["test_dir"].upper()
            or "V2XSET" in hypes["test_dir"].upper()
        )
        else False
    )
        ...
        with torch.no_grad():
            ...
            if opt.fusion_method == "late":
                ...
                raise NotImplementedError(
                    "Only single, no, early, late and intermediate"
                    "fusion is supported."
                )
            ...
            if getattr(opencood_dataset, "heterogeneous", False):
                ...
                infer_result.update(
                    {"cav_box_np": cav_box_np, "lidar_agent_record": lidar_agent_record}
                )
