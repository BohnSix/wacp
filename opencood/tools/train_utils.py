...
def backup_script(full_path, folders_to_save=["models", "data_utils", "utils", "loss", 'extensions', 'tools']):
...
def load_saved_model(saved_path, model, finetune_flag=False):
    """
    ...
    if file_list:
        ...
        if finetune_flag is True:
            print("# reset loading epoch to 0 for finetune")
            initial_epoch = 0
        else:
            initial_epoch = eval(file_list[0].split("/")[-1].rstrip(".pth").lstrip("net_epoch_bestval_at"))
...
def create_model(hypes):
    ...
    if communication_mode is not None:
        backbone_config['communication_mode'] = communication_mode
...
def setup_optimizer(hypes, model, optimizer_params=None):
    """
